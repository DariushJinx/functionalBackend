...
const createOff = Joi.object({
  code: Joi.string().error(
    createHttpError.BadRequest("کد ارسال شده صحیح نمی باشد")
  ),
  percent: Joi.string()
    .min(0)
    .max(100)
  ...
  max: Joi.number()
    ...
    .error(
      createHttpError.BadRequest(
        " تعداد کاربران ارسالی جهت تخفیف صحیح نمی باشد"
      )
    ),
});
...
const getOneOff = Joi.object({
  ...
  product: Joi.string().error(
    createHttpError.BadRequest("شناسه محصول جهت تخفیف صحیح نمی باشد")
  ),
});
...
const setDiscountOnAll = Joi.object({
  discount: Joi.number()
    ...
    .max(100)
    .error(
      createHttpError.BadRequest("تخفیف ارسالی برای تمام محصولات صحیح نمی باشد")
    ),
});
